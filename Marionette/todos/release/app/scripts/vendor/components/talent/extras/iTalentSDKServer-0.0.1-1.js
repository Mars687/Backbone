	...
	var iTalentSDK = {
		...
		,_changeUrl : function(data){
			...
			if(hash.indexOf(iframeId) != -1) {
				...
				hash = hash.replace(regex, iframeId + "="+ iframeHref + "&");
		...
		,_showPopup : function(data) {
			if(window.top != window) {
				...
				var ifrmEle = this._createPopIframe(url, data['width'], data['height']);
		...
		,_createPopIframe : function(url, width, height){
			//add widgetId for popup iframe if is not exist
			var queryObject = this._getQueryObjectByHash(url, false);
			queryObject['widgetId'] = queryObject.widgetId ? queryObject.widgetId : 'popup_' + (new Date()).getTime() + parseInt(Math.random(1,10)*3000);

			var url = this._replaceUrlQueryString(url, queryObject);
			...
			ifrmEle.setAttribute('src', url);
			ifrmEle.setAttribute('id', queryObject['widgetId']);
		...
		,_replaceUrlQueryString: function(url, queryObject) {
			var queryString = '?';
