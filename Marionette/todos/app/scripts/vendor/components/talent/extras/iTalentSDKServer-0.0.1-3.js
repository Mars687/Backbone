	...
	*/
	...
	var iTalentSDK = {
		...
		,_addAppNameToEventName(event_name){
			...
			return window.top.BSGlobal.appName ? window.top.BSGlobal.appName+"_" + event_name : event_name
		}
		,popUp : function(){
		...
		,_showPopup : function(data) {
			if(window.top != window) {
				...
				var ifrmEle = this._createPopIframe(url, data['width'], data['height'], data.publishData);
		...
		,_createPopIframe : function(url, width, height, data){
			...
			ifrmEle.setAttribute('src', location.protocol + '//' + location.host + '/italent-transfer-data.html');
