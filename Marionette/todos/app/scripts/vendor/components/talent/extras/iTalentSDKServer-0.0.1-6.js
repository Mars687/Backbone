	...
	var iTalentSDK = {
		...
		,_setIframeAminateAttr : function(_childPosition,ifrmEle){
			...
			if(_childPosition && _childPosition.top !='undefined' && _childPosition.left !='undefined'){
				...
				ifrmEle.setAttribute('style','position:fixed;transform:scale(0.1);background-color:#fff;border-radius: 2px!important;overflow:hidden;transform-origin:left top;transition:transform 0.5s cubic-bezier(0.25, 0.1, 0.25, 1), top 0.5s cubic-bezier(0.25, 0.1, 0.25, 1), left 0.5s cubic-bezier(0.25, 0.1, 0.25, 1); z-index: '+ index +';top:'+top+'px;left:'+left+"px");
			
			}else{
				
				ifrmEle.setAttribute('style','position:fixed;left:50%;transform:translate(-50%, -50%);background-color:#fff;opacity: 1;border-radius: 2px!important;overflow:hidden;transition:top 0.5s cubic-bezier(0.44, 1.53, 0.32, 0.92), transform 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);top:-800px; z-index: '+ index +';');
			}
			return ifrmEle;
		}
