	...
	*/
	...
	var iTalentSDK = {
		register : function(registers) {
			...
			function onKeyDown(e){
				...
				if(kc ==27){
					...
					message._iTalentKeyCode = 27; //esc
		...
		,bindHashChangeEvent : function() {
		    ...
		    function update(e){
		    	self.recodeIframeUrl(iframeId);
		    }
		}
		,recodeIframeUrl : function(iframeId) {
			var self = this;
			var data = data || {};
		...
		,_changeUrl : function(data){
			...
			if(hash.indexOf(iframeId) != -1) {
				...
				if(window.location.hash != hash) {
					//去掉_italentHashNotChange了，合并if else
					if(window.history.pushState) {
						window.history.pushState(null, null,"#" + hash);
					}else if(window.Talent){
						//history:navigate 不能设为true,如果为true,italent会刷新两次
		...
		,_onloadPopUpAnimate : function(e,_childPosition,iframeData,url){
			...
			if(_childPosition){
				...
				setTimeout(function(){
					var iframeAnimateAttr = self.getIframeAnimateAttr(false,true);
					iframe.setAttribute('style',iframeAnimateAttr + 'z-index:'+iframe.style.zIndex);
				},200);
			}
		...
		,_setIframeAminateAttr : function(_childPosition,ifrmEle){
			...
			if(_childPosition && _childPosition.top !='undefined' && _childPosition.left !='undefined'){
				...
				ifrmEle.setAttribute('style',iframeAnimateAttr+ 'z-index: '+ index +';top:'+top+'px;left:'+left+'px');
			
			}else{
				var iframeAnimateAttr = this.getIframeAnimateAttr(false,false);
				ifrmEle.setAttribute('style',iframeAnimateAttr + "z-index:"+index);
			}
			return ifrmEle;
		}
		,getIframeAnimateAttr : function (isHaveChild,isAnimate){
			...
			var noChildIframeInitStyle = "position:fixed;left:50%;transform:translate(-50%, -50%);-webkit-transform:translate(-50%,-50%);-moz-transform:translate(-50%,-50%);-ms-transform:translate(-50%,-50%);-o-transform:translate(-50%,-50%);opacity: 1;overflow:hidden;transition:top 0.5s cubic-bezier(0.44, 1.53, 0.32, 0.92), transform 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);-webkit-transition:top 0.5s cubic-bezier(0.44, 1.53, 0.32, 0.92), transform 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);-moz-transition:top 0.5s cubic-bezier(0.44, 1.53, 0.32, 0.92), transform 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);-o-transition:top 0.5s cubic-bezier(0.44, 1.53, 0.32, 0.92), transform 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);-ms-transition:top 0.5s cubic-bezier(0.44, 1.53, 0.32, 0.92), transform 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);top:-800px;";
			var noChildIframeAnimateStyle = "position:fixed;left:50%;transform:translate(-50%, -50%);-webkit-transform:translate(-50%,-50%);-moz-transform:translate(-50%,-50%);-ms-transform:translate(-50%,-50%);-o-transform:translate(-50%,-50%);opacity: 1;overflow:hidden;transition:top 0.5s cubic-bezier(0.44, 1.53, 0.32, 0.92), transform 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);-webkit-transition:top 0.5s cubic-bezier(0.44, 1.53, 0.32, 0.92), transform 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);-moz-transition:top 0.5s cubic-bezier(0.44, 1.53, 0.32, 0.92), transform 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);-o-transition:top 0.5s cubic-bezier(0.44, 1.53, 0.32, 0.92), transform 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);-ms-transition:top 0.5s cubic-bezier(0.44, 1.53, 0.32, 0.92), transform 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);top:50%;";
			
			if(!isHaveChild && !isAnimate){
				return noChildIframeInitStyle;
			}else if(!isHaveChild && isAnimate){
